...
`Request Example <{{version}}/examples/{{request['operationId']}}_req.json>_`
{%- endif %}
{% if request['examples']['text/plain'] %}
`Request Example <{{version}}/examples/{{request['operationId']}}_req.txt>_`
{%- endif %}
{% for status_code, response in request.responses.items() %}
{%- if response['examples']['application/json'] %}
`Response Example {{status_code}} <{{version}}/examples/{{request['operationId']}}_resp_{{status_code}}.json>_`
{%- endif %}
{%- if response['examples']['text/plain'] %}
`Response Example {{status_code}} <{{version}}/examples/{{request['operationId']}}_resp_{{status_code}}.txt>_`
{%- endif %}
{% if response['schema']['$ref'] %}
`Response Schema {{status_code}} <{{version}}/{{response['schema']['$ref'].rsplit('/', 1)[1]}}.json>_`
{%- endif %}
{% endfor %}
...
{% if parameter.schema %}
`Request Schema: <{{version}}/{{request['operationId']}}.json>_`
{%- endif -%}
...
{{ parameter|format_param('header') }}
{% endif %}
{%- endfor %}
{% for status_code, response in request.responses.items() %}
Status Code: {{status_code}} {{response.description}}
{%- endfor %}
...
# TODO, clean-up description formatting, generate rst table
...
environment.filters['double_underline'] = double_underline
