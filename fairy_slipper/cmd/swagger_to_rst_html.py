...
def write_rst_index(swagger, output_dir):
    info = swagger['info']
    version = info['version']
    service = info['x-service']
    output_file = 'index.rst'
    filepath = path.join(output_dir, output_file)
    log.info("Writing index for rst publishing %s", filepath)

    # Figure out what to write to index
    # index['/'.join([service, version, ''])] = info
    # with codecs.open(filepath,
    #                 'w', "utf-8") as out_file:
...
def write_rst(swagger, output_dir):
    ...
    # Investigate what to do with the descriptions
...
def write_examples(swagger, output_dir):
    ...
    full_path = path.join(versioned_path, 'x-examples')
    if not path.exists(service_path):
        os.makedirs(service_path)
