    ...
    def startElement(self, name, _attrs):
        ...
        if name == 'wadl:doc':
            ...
            if self.on_top_tag_stack('method'):
                # Swagger: title added as extension
                self.current_api['x-title'] = attrs.get('title')
        ...
        if name == 'xsdxt:code':
            ...
            if sample and type == 'response':
                ...
                if 'operationId' in self.current_api:
                    schema_name = self.current_api['operationId']
                    if schema_name not in self.request_example_schemas:
                        self.request_example_schemas[schema_name] \
                            = {'example': {}}
                        req_schema = self.request_example_schemas[schema_name]
        ...
        if self.on_top_tag_stack('request', 'representation', 'param'):
            ...
            if parameter['in'] == 'body':
                ...
                if parameter['required'] is True:
                    ...
                    schema_required = self.schemas[schema_name]['required']
                    if name not in schema_required:
                        schema_required.append(name)
                schema_properties[parameter['name']] = parameter

                # Request example
                if schema_name in self.request_example_schemas:
                    schema = self.request_example_schemas[schema_name]
                    self.schemas[schema_name]['example'] = schema['example']
                del parameter['required']
                del parameter['name']
                del parameter['in']
            elif parameter['in'] == 'path':
                # Swagger spec: set required field to true
                parameter['required'] = True
        ...
        if self.on_top_tag_stack('response', 'representation', 'param'):
            ...
            if parameter['in'] == 'body':
                ...
                # No required field for header object
...
def main1(source_file, output_dir):
    ...
    # Swagger: No summary field, add as extension
    tag_array = []
    for i, tag in enumerate(api_ref['tags']):
        tag_array.append(
